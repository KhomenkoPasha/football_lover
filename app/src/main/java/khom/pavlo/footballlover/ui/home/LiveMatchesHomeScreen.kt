...
fun LiveMatchesHomeScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.loadLiveMatches(forceRefresh = true)
    }
    ...
    Column(
        ...
        state.liveMatchesError?.let { error ->
            Text(text = error, color = MaterialTheme.colorScheme.error)
        }

        if (!state.isLoadingLiveMatches && state.liveMatches.isEmpty()) {
            Text(
                text = stringResource(R.string.msg_no_live_matches_now),
...
private fun LiveMatchRow(
    ...
    ElevatedCard(
        ...
        ListItem(
            ...
            overlineContent = { Text(text = event.leagueName ?: "—") },
            ...
            leadingContent = {
                LiveTeamBadge(event.homeTeamBadgeUrl, event.homeTeam)
            },
            trailingContent = {
                Row(
                    ...
                    LiveTeamBadge(event.awayTeamBadgeUrl, event.awayTeam)
                }
            },
...
@Composable
private fun LiveTeamBadge(badgeUrl: String?, description: String?) {
